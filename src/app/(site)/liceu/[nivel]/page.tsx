...
export function generateStaticParams() {
  return getLevelsByGroup("liceu").map((level) => ({ nivel: level.grade }))
}
...
export default async function HighLevelPage({
  ...
  const section = getSectionByGroup("liceu")

  if (!level || level.group !== "liceu") {
    notFound()
  }
  ...
  return (
    <div className="space-y-6">
      <SiteBreadcrumbs items={[{ href: section?.href, label: section?.title ?? t("nav.high") }, { label: level.title }]} />
      <Card>
        <CardHeader>
